...
## Pipeline Maintenance & Cleanup

The tracker (`data/applications.md`) accumulates structural drift over time — column count mismatches, broken report links, orphan reports, duplicate tracker IDs. Run `node verify-pipeline.mjs` regularly to detect issues. Common fixes below; full recipes in `references/pipeline-cleanup.md`.

### Common Tracker Issues

| Symptom | Cause | Fix |
|---------|-------|-----|
| `verify-pipeline.mjs` reports orphan reports | Report file exists but no tracker row links to it (stale re-eval duplicates) | Delete the stale report file if a newer report for the same company+role exists |
| Broken report links (`❌ Report not found`) | Tracker row references a report number that no longer exists | Replace broken link with `❌` in the Report column |
| Row has 11 columns (missing Deadline) | Older batch format omitted the Deadline column | Insert empty Deadline column at index 7: `parts[0..6] + [''] + parts[7..]` |
| Row has 13 columns (extra empty) | Trailing pipe on 12-column format | Remove trailing empty column |
| Row starts with ` \|` (space-pipe) | Formatting drift from manual edits | Normalize to `\|` |
| Duplicate tracker number N | Re-evaluation reused the same tracker number | Renumber the newer row to a fresh high number (e.g., 2000+) |

### Critical: Normalize Before verify-pipeline.mjs

The verifier parses rows by splitting on `|` and trimming. Rows with column-count mismatches silently corrupt column indexing — the Report column ends up at the wrong position, masking real orphans and creating false positives. Always normalize column counts first:

```bash
# Fix 11-column rows (add empty Deadline at index 7)
# Fix 13-column rows (remove trailing empty)
# Normalize ' |' → '|'
# Then re-run verify-pipeline.mjs
```

### Batch Input File Format

`batch/batch-input.tsv` MUST use actual tab characters (`\t`), not literal `\t` (backslash-t). Files downloaded from some sources (e.g., spreadsheet exports) may contain escaped tabs. Verify with:

```bash
# Check for literal backslash-t (BAD)
grep -c '\\t' batch/batch-input.tsv

# Check for real tabs (GOOD)
awk -F'\t' '{print NF}' batch/batch-input.tsv | sort | uniq -c
```

Fix escaped tabs with: `sed -i '' 's/\\t/\t/g' batch/batch-input.tsv`

### Protecting User-Layer Files from Updates

`update-system.mjs apply` prunes system files that differ from upstream and aren't in `USER_PATHS`. To protect a custom file, add it to `config/local-paths.txt` (gitignored, one path per line):

```
providers/management-consulted.mjs
```

Move user-owned templates/directories into `documents/` (a `USER_PATHS` entry) so they're never auto-updated. Example: `templates/BYU-Specific/` → `documents/templates/BYU-Specific/`.

### Recovering from a Broken `update-system.mjs` Run

If the update fails on `git fetch` with `fatal: bad object refs/heads/main 2`:
1. Remove the stale ref: `rm ".git/refs/heads/main 2"` (note the space in the filename)
2. Re-run `node update-system.mjs apply`

If `.git/index.lock` blocks the commit:
1. `rm .git/index.lock`
2. The update's file checkout already ran; manually `git commit` the staged changes

### Batch Runner Permission Modes by CLI

| CLI | Permission Bypass Flag |
|-----|----------------------|
...
The default batch CLI is AGY (`--cli agy`). Claude is the fallback when AGY quota is exhausted. Always verify Claude auth with `claude auth status` before running a Claude batch.
