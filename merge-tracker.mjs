...
function validateStatus(status) {
  ...
  // Aliases
  const aliases = {
    applied: 'Applied', submitted: 'Applied', sent: 'Applied',
    closed: 'Discarded', cancelled: 'Discarded', canceled: 'Discarded',
    duplicate: 'Discarded', repost: 'Discarded',
    no_apply: 'SKIP', 'no apply': 'SKIP', skip: 'SKIP', monitor: 'SKIP', hold: 'SKIP',
    to_apply: 'Evaluated', 'to apply': 'Evaluated', watch: 'Evaluated', watching: 'Evaluated', under_review: 'Evaluated',
    geo_blocker: 'SKIP', 'geo blocker': 'SKIP',
  };
...
function parseTsvContent(content, filename) {
  ...
  if (content.startsWith('|')) {
    ...
    const col4LooksLikeStatus = /^(evaluated|applied|responded|interview|offer|rejected|discarded|skip|to apply|watch|closed|duplicate|repost|hold|monitor)/i.test(col4);
    const col5LooksLikeStatus = /^(evaluated|applied|responded|interview|offer|rejected|discarded|skip|to apply|watch|closed|duplicate|repost|hold|monitor)/i.test(col5);
